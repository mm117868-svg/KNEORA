...
test('missing and low confidence landmarks rejected',()=>{assert.equal(measure(null,1920,1080,'left'),null);assert.equal(measure(Array.from({length:33},()=>({x:.5,y:.5,visibility:.1})),1920,1080,'left'),null);});
...
test('hidden shoulder preserves knee only and low presence is rejected',()=>{const p=pose();p[11].visibility=.1;let r=assessPose([p],1000,600,'left');assert.equal(r.frameStatus,'partial');assert.equal(r.bend,0);assert.equal(r.hipAngle,null);p[25].presence=.1;r=assessPose([p],1000,600,'left');assert.equal(r.frameStatus,'rejected');assert.match(r.rejectionReasons,/knee:low_presence/);});
