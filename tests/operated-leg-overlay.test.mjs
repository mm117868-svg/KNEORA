...
import {FluidOutline, PictureClock, drawOutline, drawHipOutline, drawToeOutline} from '../fluid-outline.mjs';
...
test('the patient screen no longer has any way to draw the whole skeleton', () => {
  ...
  assert.match(html, /aimOutline\(previewLeg, previewAll, previewLastVideoTime\)/);
  assert.match(html, /aimOutline\(trace\.lastLm, lastAll, lastVideoTime\)/);
});
