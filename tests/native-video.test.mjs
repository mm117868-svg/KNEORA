...
test('changed modules carry new version tags, so a cached copy is not mixed with a new page', () => {
  ...
  assert.equal(tag(index, 'recovery-summary.mjs'), 'scatter-1');
  ...
  for (const [file, expected] of [['fluid-outline.mjs', 'exercise-landmarks-4'], ['confidence.mjs', '1'], ['recovery-measurements.mjs', 'interval-1'], ['recovery-trends.mjs', 'scatter-1'], ['recovery-camera.mjs', 'wave-1']]) {
