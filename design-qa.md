...
- Measurement safety remains active silently. Acceptance now uses a 20% landmark-visibility threshold, tolerates a 25% frame-to-frame jump and retains a sample for up to 1.2 seconds. Frames that still fail these checks are excluded, but the warning UI does not interrupt or stop the recording.
- The dedicated MediaPipe hand-pose model now reads Intel colour frames directly and the two-second open-palm hold is timed from those frame arrivals. A clearly extended hand can also start the countdown when that same model's gesture classifier returns an uncertain result, while an identified fist or weak palm result is rejected.
- Mini squat and squat overlays show only the operated knee point. Trunk, hip, ankle, heel, toe, face and hand pose landmarks are not drawn.
- Display tracking accepts lower-visibility landmarks and keeps the last reliable position through brief MediaPipe gaps. The live knee number is averaged and rate-limited to reduce flicker, responds faster during real movement and keeps the most recent valid value marked as `last reading` when tracking is unavailable.
