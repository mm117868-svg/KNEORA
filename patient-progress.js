...
export function calendarMonth(year, month, records, operationDate, today = new Date(), events = []) {
  ...
  return Array.from({length:size},(_,i)=>{
    ...
    return {key,day:d.getDate(),inMonth:d.getMonth()===month,today:key===dayKey(today),surgery:key===operationDate,records:used.get(key)||[],events:events.filter(e=>e.date===key)};
  });
}
function renderCalendar(host, records, operationDate, onReview, initialMonth, options) {
  ...
  const reminders=calendarFollowups({patientId:options.patientId,operationDate,today:dayKey(today),storage:options.storage});
  ...
  function draw(focusAction) {
    const cells=calendarMonth(month.getFullYear(),month.getMonth(),records,operationDate,today,reminders.events);
...
function calendarEventLink(event) {
  return `<a href="${followupLink(event)}">${escapeHtml(event.title)} · ${escapeHtml(event.side)} knee · Day ${event.day} · ${escapeHtml(parseDay(event.date).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}))}<small>${escapeHtml(event.status)}</small></a>`;
}
function calendarEventList(reminders, month) {
  const events=reminders.events.filter(e=>e.date.startsWith(month));
  return `<div class="calendar-followups"><h4>Questionnaire follow-ups</h4>${events.map(calendarEventLink).join('')||'<small>No questionnaire follow-ups planned this month.</small>'}${reminders.error?`<p role="status">${escapeHtml(reminders.error)}</p>`:''}<a href="?view=patient-measures#questionnaire-followups">Manage follow-up dates →</a></div>`;
}
...
export function renderHomeActivity(element, records, patientId, operationDate) {
  ...
  const reminders=calendarFollowups({patientId,operationDate,today:dayKey(today)});
  let month=new Date(today.getFullYear(),today.getMonth(),1,12);
  function draw(focus){
    const days=calendarMonth(month.getFullYear(),month.getMonth(),[],operationDate,today,reminders.events);
    element.classList.add('home-activity');
    element.setAttribute('aria-label','App activity calendar');
    element.innerHTML=`<div class="mini-calendar"><div class="mini-calendar-head"><h2 aria-live="polite">${escapeHtml(month.toLocaleDateString('en-GB',{month:'long',year:'numeric'}))}</h2><div><button type="button" data-mini-nav="-1" aria-label="Previous month">‹</button><button type="button" data-mini-nav="1" aria-label="Next month">›</button></div></div><div class="mini-calendar-grid" role="group" aria-label="Days this app was used">${['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(d=>`<span class="mini-weekday" aria-label="${d}">${d[0]}</span>`).join('')}${days.map(d=>d.inMonth?`<${d.events.length?'a':'span'}${d.events.length?` href="${d.events.length===1?followupLink(d.events[0]):'?view=patient-measures#questionnaire-followups'}"`:''} class="mini-day${d.events.length?' followup':''}${visits.includes(d.key)?' visited':''}${d.today?' today':''}${d.surgery?' surgery':''}" aria-label="${escapeHtml(parseDay(d.key).toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'}))}${visits.includes(d.key)?', app used':''}${d.today?', today':''}${d.surgery?', surgery date':''}${d.events.length?', '+d.events.map(e=>e.title+' questionnaire, '+e.status).join('; '):''}"${d.today?' aria-current="date"':''}>${d.day}</${d.events.length?'a':'span'}>`:'<span aria-hidden="true"></span>').join('')}</div><div class="mini-calendar-legend"><span><i></i>App used</span><span>○ Today</span>${parseDay(operationDate)?'<span><i class="surgery-dot"></i>Surgery date</span>':''}${reminders.events.length?'<span><i class="questionnaire-dot"></i>Questionnaire</span>':''}</div></div><p class="mini-postop" aria-live="polite">${escapeHtml(postop)}</p><p class="mini-calendar-note">${saved?'Days opened or with a saved session on this device.':'Activity could not be saved on this device.'}</p><a class="mini-tracker-link" href="?view=progress#progress">Open progress tracker →</a><div class="mini-followup-note">${reminders.events.filter(e=>!e.result).slice(0,2).map(calendarEventLink).join('')}${reminders.error?escapeHtml(reminders.error):''}<a href="?view=patient-measures#questionnaire-followups">${reminders.events.length?'Manage questionnaire follow-ups':'Add a questionnaire follow-up'} →</a></div>`;
