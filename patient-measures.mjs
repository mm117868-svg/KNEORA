...
export function renderPatientMeasures(host,options={}) {
 ...
 host.innerHTML=`<div class="pt-heading"><div class="eyebrow">Your recovery · questionnaires</div><h1>Patient-reported measures</h1><p>How your knee feels and functions in everyday life, recorded on each assessment date.</p><p class="pt-context">Patient ${esc(patientId)} · ${operationDate?`Operation ${esc(shortDate(operationDate))}`:'Operation date not set'} · <a href="?view=settings#patient">Change patient details</a></p></div>
...
export function renderFollowupSchedule(host, options={}) {
 ...
 host.innerHTML=`<h2>Questionnaire follow-ups</h2><p>Add the days agreed with your clinical team. Each reminder appears on your calendar with its date. Surgery is day 0.</p>${ready?'':'<p><a href="?view=settings#opdate">Enter your operation date in Settings</a> to calculate follow-up dates.</p>'}
