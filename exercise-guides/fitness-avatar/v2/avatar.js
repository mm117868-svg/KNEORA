...
export async function makeFitnessAvatar(canvas){
 ...
 a.renderer.toneMapping=T.ACESFilmicToneMapping;a.renderer.toneMappingExposure=1.1;
 a.scene.background=new T.Color('#f0ebe3');a.scene.traverse(o=>{if(o.isLight)o.intensity*=.7;});
 a.chair.clear();
 const wood=new T.MeshStandardMaterial({color:'#b79875',roughness:.72}),fabric=new T.MeshStandardMaterial({color:'#ded5c7',roughness:.92});
 function rounded(w,h,d,x,y,z,mat){const shape=new T.Shape();shape.moveTo(-w/2,-h/2);shape.lineTo(w/2,-h/2);shape.lineTo(w/2,h/2);shape.lineTo(-w/2,h/2);shape.closePath();const g=new T.ExtrudeGeometry(shape,{depth:d,bevelEnabled:true,bevelSegments:4,steps:1,bevelSize:.012,bevelThickness:.012});g.translate(0,0,-d/2);const m=new T.Mesh(g,mat);m.position.set(x,y,z);m.castShadow=m.receiveShadow=true;a.chair.add(m);return m;}
 rounded(.47,.055,.49,-.38,.445,0,fabric);rounded(.042,.36,.46,-.62,.68,0,fabric);
 for(const x of [-.58,-.17])for(const z of [-.22,.22]){const m=new T.Mesh(new T.CylinderGeometry(.022,.014,.43,20),wood);m.position.set(x,.212,z);m.castShadow=m.receiveShadow=true;a.chair.add(m);}
 a.scene.traverse(o=>{if(o.isDirectionalLight&&o.castShadow){o.shadow.radius=6;o.shadow.mapSize.set(2048,2048);o.intensity=.8;o.position.set(-3,6,5);}if(o.isHemisphereLight)o.intensity=2.3;if(o.isMesh&&o.geometry.type==='PlaneGeometry')o.material.color.set('#e5dfd5');});
 const softFill=new T.DirectionalLight('#fff3e5',1.1);softFill.position.set(4,3,6);a.scene.add(softFill);
 const gltf=await new GLTFLoader().loadAsync(new URL('./fitness-avatar.glb',import.meta.url).href);
 ...
 function pose(id,t){
  ...
  if(id==='seated_extension'){
   ...
   a.camera.position.set(4.3,1.85,8);a.camera.lookAt(.03,.70,0);a.camera.updateProjectionMatrix();
  }
