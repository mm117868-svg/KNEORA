...
macro=TargetService.get_default_macro_info_dict();macro.update(gender=1.0,age=.72,muscle=.50,weight=.46,height=.50,proportions=.5);macro['race']={'caucasian':1.,'asian':0.,'african':0.}
body=HumanService.create_human(macro_detail_dict=macro);body.name='FitnessAvatarBody'
TargetService.load_target(body,str(root/'mpfb2/src/mpfb/data/targets/mouth/mouth-angles-up.target.gz'),weight=.55)
assets=root/'assets'
...
m.node_tree.nodes['Principled BSDF'].inputs['Base Color'].default_value=(.07,.08,.085,1)
...
for path,color in [('clothes/toigo_basic_tucked_t-shirt/toigo_basic_tucked_t-shirt.mhclo',(.13,.235,.25,1))]:
...
sub=body.modifiers.new('Smooth skin','SUBSURF');sub.levels=2;sub.render_levels=2
...
for m in bpy.data.materials:
 ...
 if '.short04' in m.name:
  ...
  if principled:
   ...
   base.default_value=(.27,.255,.235,1)
# Neutral trainers replace the dark shoe texture while retaining shoe geometry.
shoes=loaded['clothes/shoes06/shoes06.mhclo']
m=bpy.data.materials.new('FitnessIvoryTrainers');m.use_nodes=True
n=m.node_tree.nodes['Principled BSDF'];n.inputs['Base Color'].default_value=(.78,.79,.77,1);n.inputs['Roughness'].default_value=.72
shoes.data.materials.clear();shoes.data.materials.append(m)
for obj in [loaded['clothes/toigo_basic_tucked_t-shirt/toigo_basic_tucked_t-shirt.mhclo'],shorts]:
 for modifier in obj.modifiers:
  if modifier.type=='SUBSURF':modifier.levels=2;modifier.render_levels=2
for material in body.data.materials:
 if material.use_nodes:
  node=material.node_tree.nodes.get('Principled BSDF')
  if node:node.inputs['Roughness'].default_value=.65
