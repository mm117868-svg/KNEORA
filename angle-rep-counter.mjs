...
export class AngleRepCounter {
  ...
  update(angle, t) {
    ...
    if (this.state === 'rest') {
      if (size >= this.enter) { this.since ??= t; if (t - this.since >= this.s.settleS) { this.state = 'away'; this.awayAt = this.since; this.peak = size; this.since = null; this.holdSince = null; this.held = 0; this.steadyFrom = null; } }
    ...
    if (size <= Math.max(this.s.exitShare * this.peak, 0.5 * this.s.minDeg)) {
      ...
      this.reps++; this.lastRepAt = t; this.peaks.push(this.peak); this.events.push({t, status: 'accepted', excursion_deg: +this.peak.toFixed(1), away_s: +held.toFixed(2), hold_s: +this.held.toFixed(2)});
      return 1;
