...
export class EndpointPreviewAverage {
 ...
 update(angle,points,now){
  ...
  this.last=now;this.frames.push({angle,points,time:now});if(this.frames.length>5)this.frames.shift();
  const mean=values=>values.reduce((a,b)=>a+b,0)/values.length;
  return {angle:mean(this.frames.map(f=>f.angle)),samples:this.frames.length,angles:this.frames.map(f=>f.angle),times:this.frames.map(f=>f.time),
