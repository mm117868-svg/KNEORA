...
The dedicated recovery measurement uses Heavy, corresponding to the legacy complexity 2 choice, through the existing Tasks Vision runtime. It uses `runningMode: 'IMAGE'` and `detect()` on each fresh camera picture or uploaded image. No pose from a previous picture is supplied to the next measurement. Live camera capture allows up to six seconds and ten pictures; it can finish early using the red button. A five-second Marin countdown precedes each capture. Each usable raw knee angle contributes once to the average and the minimum/maximum range. At least three clear pictures and 20% measurable coverage are needed, with a 20% joint visibility threshold. Large angle variation adds a note without rejecting the result. Missing joints, ambiguous multiple people and invalid geometry remain excluded. These are prototype processing choices, not clinical acceptance thresholds. Imported depth checks retain the previous rules. Images are processed in memory on this device; only measurements are saved when the patient confirms Save.
