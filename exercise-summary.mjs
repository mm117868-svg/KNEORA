...
export function renderBasicExerciseSummary(record,history=[],{details=true}={}) {
 ...
 <div class="exercise-simple-context"><h3>For your recovery${s.day!==null&&s.day>=0?` on day ${s.day}`:''}</h3><p>${esc(s.focus)}</p>${s.comparison?`<p>${esc(s.comparison)}</p>`:''}<p class="exercise-simple-note">Follow your own plan. Recovery has no single daily pass mark.</p><a href="?view=progress-to-date">See your physiotherapy feedback</a>${s.source?` · <a href="${esc(s.source)}" target="_blank" rel="noopener">Exercise reference</a>`:''}</div>
