...
The overlay is specific to the exercise. Heel slides and seated knee extensions show the operated hip, knee and ankle, with the live knee angle at the knee. Straight leg raises add the operated-side shoulder for the trunk reference and the operated heel and toe for foot direction. Its live displays show knee bend, hip raise and how far the heel-to-toe line differs from vertical-up in the camera picture. Zero degrees on the toe display means the toes point towards the top of a level picture. It is a camera cue, not a clinical foot or ankle measurement. The pose model still returns the whole body, but the other leg, face and unrelated upper-body landmarks are never drawn. Before recording, a separate MediaPipe Gesture Recognizer checks for `Open_Palm` at 8 images per second. Holding an open palm for two seconds starts the countdown without needing the face, shoulder or body pose. The hand model stops being sampled once recording begins, so it does not compete with exercise measurement. A raised hand or wave can still finish the session using the pose result already required for the leg, and the Start button and space bar remain available. `tests/operated-leg-overlay.test.mjs` runs the real preview loop with a whole-body result and checks where ink lands.
