...
export function measurementSeries(r, metric) {
 ...
 const side=metric.source==='video'?c.side:r.measurement?.side;
 ...
 if(metric.source==='live')base.push(side||'unknown',r.count_source||'monitoring',!!r.hold,r.measurement?.stats_min_visibility??null);
...
export function seriesFor(records,exercise,metric) {
 ...
 for(const r of records.filter(r=>r.exercise===exercise).sort((a,b)=>Date.parse(a.started_at)-Date.parse(b.started_at))){
  ...
  const key=measurementSeries(r,metric),a=report(r),side=metric.source==='video'?a?.config?.side:r.measurement?.side;
