...
export class LivePositionCounter {
 ...
 reset(){this.cycle=new PositionCycle();this.reference=null;this.samples=[];this.start=null;this.last=null;this.message=!['straight_leg_raise','seated_extension'].includes(this.exercise)?'Position model not yet trained for this exercise. Recording is available.':!this.model?'Position model unavailable. No repetitions will be estimated.':'Keep your leg resting and still for 3 seconds.';}
 get supported(){return ['straight_leg_raise','seated_extension'].includes(this.exercise)&&!!this.model&&(!this.model.exercise?this.exercise==='straight_leg_raise':this.model.exercise===this.exercise);}
 ...
 update(frame,t){
  ...
  const p=predict(this.model,frame,this.reference);this.cycle.update(p,t);this.message=Math.max(...p)<.75?'Position uncertain. This movement may remain unconfirmed.':this.cycle.state==='raised'?'Leg raised. Lower to the resting position.':'Resting or moving between positions.';
 }
