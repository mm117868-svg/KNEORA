...
OUT=ROOT/'position-recogniser';OUT.mkdir(exist_ok=True)
...
train=next(c for c in manifest if c['id']=='01.39.33');rest=gray(train['frames'][0]['image']);X=[];Y=[];annotated=[]
for t,y in labels:
...
model.eval()
weights={'version':'slr-position-v1','classes':['rest','intermediate','raised'],'input':{'width':32,'height':32,'crop':[0,.12,1,.78],'excluded_left_columns':10,'features':'signed grayscale difference from starting image, median brightness corrected, clipped to [-0.5,0.5]'},'hidden':24,'w1':model[0].weight.detach().numpy().tolist(),'b1':model[0].bias.detach().numpy().tolist(),'w2':model[2].weight.detach().numpy().tolist(),'b2':model[2].bias.detach().numpy().tolist(),'training_clip':'01.39.33','training_label_count':len(labels),'augmentation':'paired rotation +/-12 degrees, scale 0.8-1.2, translation +/-4 pixels, brightness','limitations':['One person and room, one training recording.','Requires a side-on lower-limb view similar to the supplied examples.','Has no learned selected-leg identity or out-of-distribution detector.','Not validated for tiny movements, other homes, occlusion or clinical use.']}
(OUT/'model.json').write_text(json.dumps(weights))
(OUT/'labels.json').write_text(json.dumps(annotated,indent=2))
report={'training_seconds':time.perf_counter()-start,'training_clip':train['id'],'validation_clip':'02.04.17','split':'whole recording, same person and room; development comparison after inspecting initial failure, not untouched validation','labels':annotated,'clips':[]}
with torch.no_grad():
 for cid in ['01.39.33','02.04.17']:
