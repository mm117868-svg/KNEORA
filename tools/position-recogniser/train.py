...
parser=argparse.ArgumentParser();parser.add_argument("--exercise",choices=["straight_leg_raise","seated_extension"],default="straight_leg_raise");exercise=parser.parse_args().exercise
from pathlib import Path
import cv2,numpy as np,torch
ROOT=Path(os.environ.get('KNEE_TRACKER_HOME',str(Path(__file__).resolve().parents[3]/'knee-dis-evaluation')))
OUT=ROOT/'position-recogniser';OUT=OUT/'seated_extension' if exercise=='seated_extension' else OUT;OUT.mkdir(parents=True,exist_ok=True)
...
with torch.no_grad():
 for cid in [train_id,comparison_id]:
